 ...
 *
 */
public class MP3DecoderModded
{
    AudioInputStream in;
    FloatSampleBuffer buffer;
    byte[] bytes;

    public MP3DecoderModded(InputStream stream ) throws Exception
    {
    ...
    public int readSamples(float[][] samples)
    {
        ...
        for( int i = 0; i <buffer.getSampleCount(); i++ )
        {
            if( buffer.getChannelCount() == 2 ) {
                samples[i][0] = buffer.getChannel(0)[i];
                samples[i][1] = buffer.getChannel(1)[i];
            } else {
                samples[i][0] = buffer.getChannel(0)[i];
                samples[i][1] = buffer.getChannel(0)[i];
            }
        }
